...
The timestamp must be within ±5 minutes of the server's current UTC clock.
...
_TIMESTAMP_WINDOW = timedelta(minutes=5)
...
def verify_device_request(event: dict, expected_uuid: str, device_item: dict) -> None:
    ...
    if delta > _TIMESTAMP_WINDOW:
        ...
        raise DeviceAuthError("X-Device-Timestamp is outside the ±5 minute acceptance window", 401)
