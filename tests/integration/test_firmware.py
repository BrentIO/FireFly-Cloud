...
def test_list_firmware_returns_200(api_url):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    assert resp.status_code == 200


def test_list_firmware_response_has_items_key(api_url):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    assert "items" in resp.json()


def test_list_firmware_items_is_list(api_url):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    assert isinstance(resp.json()["items"], list)


def test_list_firmware_content_type(api_url):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    assert "application/json" in resp.headers.get("Content-Type", "")


def test_list_firmware_item_excludes_files_field(api_url, firmware_item):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    for item in resp.json()["items"]:
        assert "files" not in item, "List response must not include 'files'"


def test_list_firmware_item_excludes_pk_field(api_url, firmware_item):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
    for item in resp.json()["items"]:
        assert "pk" not in item, "List response must not include 'pk'"


def test_list_firmware_item_required_fields(api_url, firmware_item):
    resp = requests.get(f"{api_url}/firmware", timeout=10)
...
def test_list_firmware_filter_by_product_id(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware",
...
def test_list_firmware_filter_by_application(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware",
...
def test_list_firmware_filter_by_version(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware",
...
def test_list_firmware_unknown_product_id_returns_empty(api_url):
    resp = requests.get(
        f"{api_url}/firmware",
...
def test_get_firmware_item_not_found(api_url):
    resp = requests.get(
...
def test_get_firmware_item_not_found_has_message(api_url):
    resp = requests.get(
        f"{api_url}/firmware/00000000-0000-0000-0000-000000000000.zip",
        timeout=10,
    )
...
def test_get_firmware_item_returns_200(api_url, firmware_item):
    resp = requests.get(
...
def test_get_firmware_item_includes_files_field(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}",
        timeout=10,
    )
...
def test_get_firmware_item_files_have_name_and_sha256(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}",
        timeout=10,
    )
...
def test_get_firmware_item_excludes_pk_field(api_url, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}",
        timeout=10,
    )
...
def test_get_firmware_item_matches_list_entry(api_url, firmware_item):
