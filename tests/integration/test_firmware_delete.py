...
def test_delete_firmware_not_found(api_url):
    resp = requests.delete(f"{api_url}/firmware/{NONEXISTENT}", timeout=10)
    assert resp.status_code == 404


def test_delete_firmware_not_found_has_message(api_url):
    resp = requests.delete(f"{api_url}/firmware/{NONEXISTENT}", timeout=10)
...
def test_delete_firmware_returns_202(api_url, fresh_firmware_item):
...
def test_delete_firmware_response_body(api_url, fresh_firmware_item):
    resp = requests.delete(
        f"{api_url}/firmware/{fresh_firmware_item['zip_name']}",
        timeout=10,
    )
...
def test_delete_firmware_already_deleted_returns_409(api_url, fresh_firmware_item):
    """Deleting a second time after the first deletion is accepted should return 409."""
    zip_name = fresh_firmware_item["zip_name"]

    # First delete — should succeed
    first = requests.delete(f"{api_url}/firmware/{zip_name}", timeout=10)
    ...
    second = requests.delete(f"{api_url}/firmware/{zip_name}", timeout=10)
    assert second.status_code == 409


def test_delete_firmware_conflict_has_message(api_url, fresh_firmware_item):
    zip_name = fresh_firmware_item["zip_name"]
...
def test_delete_released_firmware_returns_409(api_url, fresh_firmware_item):
    ...
    resp = requests.delete(f"{api_url}/firmware/{zip_name}", timeout=10)
