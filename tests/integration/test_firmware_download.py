...
def test_download_not_found_returns_404(api_url, auth_headers):
    resp = requests.get(f"{api_url}/firmware/{NONEXISTENT}/download", headers=auth_headers, timeout=10)
    assert resp.status_code == 404


def test_download_not_found_has_message(api_url, auth_headers):
    resp = requests.get(f"{api_url}/firmware/{NONEXISTENT}/download", headers=auth_headers, timeout=10)
...
def test_download_returns_200(api_url, auth_headers, firmware_item):
    resp = requests.get(
...
def test_download_response_has_url(api_url, auth_headers, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}/download",
        headers=auth_headers,
        timeout=10,
...
def test_download_response_has_expires_in(api_url, auth_headers, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}/download",
        headers=auth_headers,
        timeout=10,
...
def test_download_expires_in_is_900(api_url, auth_headers, firmware_item):
    resp = requests.get(
        f"{api_url}/firmware/{firmware_item['zip_name']}/download",
        headers=auth_headers,
        timeout=10,
...
def test_download_url_is_accessible(api_url, auth_headers, firmware_item):
...
def test_download_url_returns_zip_content(api_url, auth_headers, firmware_item):
...
def test_download_deleted_firmware_returns_410(api_url, auth_headers, fresh_firmware_item):
...
def test_download_deleted_firmware_has_message(api_url, auth_headers, fresh_firmware_item):
    zip_name = fresh_firmware_item["zip_name"]
