...
def test_patch_status_not_found(api_url):
    resp = requests.patch(
...
def test_patch_status_not_found_has_message(api_url):
    resp = requests.patch(
        f"{api_url}/firmware/{NONEXISTENT}/status",
        json={"release_status": "TESTING"},
        timeout=10,
...
def test_patch_status_missing_release_status_field(api_url, firmware_item):
    resp = requests.patch(
...
def test_patch_status_missing_body(api_url, firmware_item):
    resp = requests.patch(
        f"{api_url}/firmware/{firmware_item['zip_name']}/status",
        timeout=10,
    )
...
def test_patch_status_invalid_transition_returns_422(api_url, firmware_item):
...
def test_patch_status_invalid_transition_response_body(api_url, firmware_item):
    resp = requests.patch(
        f"{api_url}/firmware/{firmware_item['zip_name']}/status",
        json={"release_status": "RELEASED"},
        timeout=10,
...
def test_patch_status_invalid_transition_allowed_transitions_correct(api_url, firmware_item):
...
def test_patch_status_ready_to_test_to_testing(api_url, fresh_firmware_item):
    resp = requests.patch(
...
def test_patch_status_response_excludes_pk(api_url, fresh_firmware_item):
    resp = requests.patch(
        f"{api_url}/firmware/{fresh_firmware_item['zip_name']}/status",
        json={"release_status": "TESTING"},
        timeout=10,
...
def test_patch_status_full_workflow(api_url, fresh_firmware_item):
